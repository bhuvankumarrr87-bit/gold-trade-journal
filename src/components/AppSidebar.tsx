  ...
  return (
    <aside className={`${collapsed ? 'w-[72px]' : 'w-[240px]'} min-h-screen bg-sidebar border-r border-sidebar-border flex flex-col shrink-0 transition-all duration-300 ease-out relative`}>
      ...
      <nav className="flex-1 px-3 mt-2 space-y-1">
        {!collapsed && <p className="text-xs font-semibold text-muted-foreground/60 uppercase tracking-[0.15em] px-3 mb-2">Navigation</p>}
        {navItems.map(item => {
      ...
      <div className={`p-4 border-t border-sidebar-border space-y-3 ${collapsed ? 'items-center flex flex-col' : ''}`}>
        {!collapsed && <p className="text-xs text-muted-foreground truncate">{user?.email}</p>}
        <button
          onClick={signOut}
          className={`flex items-center ${collapsed ? 'justify-center' : 'gap-2'} text-sm text-muted-foreground hover:text-foreground transition-all duration-200 w-full group`}
        >

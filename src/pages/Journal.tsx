...
export default function Journal() {
  ...
  const [journal, setJournal] = useState({ pre_trade_notes: '', post_trade_notes: '', emotions: '', lessons: '', tags: '', rating: 5, risk_reward: '1:2' });
  ...
  useEffect(() => {
    if (existingJournal) {
      setJournal({ pre_trade_notes: existingJournal.pre_trade_notes || '', post_trade_notes: existingJournal.post_trade_notes || '', emotions: existingJournal.emotions || '', lessons: existingJournal.lessons || '', tags: existingJournal.tags || '', rating: existingJournal.rating || 5, risk_reward: existingJournal.risk_reward || '1:2' });
    } else {
              ...
              <div className="bg-secondary/30 rounded-xl p-5 border border-border/40 flex items-center gap-6">
                ...
                <div className="flex items-center gap-2">
                  <input value={journal.risk_reward.split(':')[0] || '1'} onChange={e => setJournal(j => ({ ...j, risk_reward: `${e.target.value}:${j.risk_reward.split(':')[1] || '2'}` }))}
                    className="w-14 bg-card text-foreground border border-border rounded-lg px-3 py-2 text-base text-center font-mono-num focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all duration-200" />
